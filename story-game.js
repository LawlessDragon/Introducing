...
function initGame() {
  ...
  window.audioSystem.isMusicPlaying = true;
...
function loadScene(sceneId) {
  ...
  window.audioSystem.playSound('transition');
  ...
  // Play scene music
  if (scene.music && window.audioSystem) {
    window.audioSystem.playBackgroundMusic(scene.music);
  }
...
function updateMusicToggleButton() {
  const musicToggle = document.querySelector('.music-toggle');
  if (musicToggle && window.audioSystem) {
    musicToggle.innerHTML = window.audioSystem.isMusicPlaying ? '🔊 Music: ON' : '🔇 Music: OFF';
  }
}
...
function checkAudioSystem() {
  if (!window.audioSystem) {
    console.warn('Audio system not initialized yet. Waiting...');
    setTimeout(checkAudioSystem, 500);
  }
}
